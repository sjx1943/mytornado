...
剩余待办事项：
1. 在商品详情页/detail/x点击联系卖家时可以自动添加该卖家为好友，并自动跳转至聊天chat_room页面并打开与该好友的聊天窗口。
2. 删除用户评价的相关元素，并在适当位置增加商品评价的输入按钮，对于自有商品的评价，卖家可以删除该评价。
3. 增加好友的拉黑机制，拉黑后无法评价对方的商品、无法添加对方为好友并发送消息。
4. 当一方删除好友后，另一方再发送消息时，原先一方在好友列表中应重新显示有该好友，只不过聊天内容清空。
5. 交易成功后，双方确认交易，卖家更新数据库中的商品数量和状态。商品数量为0时，商品自动下架，用户在“我的”页面可以对上传商品进行状态变更、上下架或删除。
6. 首页“编辑“按钮点击无响应，可以考虑删除或增加相应自有商品的编辑功能。
7. 增加游客访问逻辑，游客可以查看商品详情、发布商品、查看活动信息等，但不能发布评论、购买商品、添加好友等。
8. 下单后的弹出的错误贴这里。
9. 美化UI界面，页面弹性布局适配PC或移动端。
10. web端和微信小程序端正式运行后，需要共用1套数据库（mysql+mongodb），注意性能优化并进行数据一致性的检查和约束。
11. 在/home_page页面，上传的商品点击“下架”后页面没有响应。
# xianyu二手平台开发部署文档
