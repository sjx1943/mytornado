...
下一步， product数据表中补充商品数量字段和商品状态（在售、已售出），主页main_page和个人商品页需要用grid-container方式分2列展示，可展示名称、商品数量和图片（图片需弹性布局，而非100%原图展示）、点击图片页面才显示原图，而不是跳转至http://localhost:8000/product/detail/x 后404 \
设计商品买卖逻辑，在主页和商品详情页中放置“想要”按钮，并发送一条提醒消息至卖家，卖家获得提醒后可调用聊天chat控制器 相互发送聊天信息，交易成功后双方确认，卖家更新数据库中的商品数量和状态。
