...
def inserData(city,pro):
    ...
    #用连接对象创建游标对象
    cursor = conn.cursor()
    sql = 'insert into tornadoAPP values(null,"%s","%s")' % (city,pro)
    cursor.execute(sql)
    conn.commit()
    conn.close()
...
def inserDatas(args=[]):
    ...
    #用连接对象创建游标对象
    cursor = conn.cursor()
    sql = 'insert into tornadoAPP values(null,"%s","%s")'
...
def queryDatasByParams(cid,city):
    ...
    #用连接对象创建游标对象
    cursor = conn.cursor()
    sql = 'select * from tornadoAPP where cid="%s" and city="%s"'%(cid,city)
    cursor.execute(sql)
    msg = cursor.fetchall()
    conn.close()
    return msg
...
def queryDatasBylike(x):
    ...
    #用连接对象创建游标对象
    cursor = conn.cursor()
    sql = 'select * from tornadoAPP where cid like "%%{re}"'.format(re=x)
    cursor.execute(sql)
    msg = cursor.fetchall()
    conn.close()
    return msg
...
def query_all_page(num,size):
    ...
    #用连接对象创建游标对象
    cursor = conn.cursor()
    sql = 'select * from tornadoAPP limit %s,%s'%((num-1)*size,size)
    cursor.execute(sql)
    msg = cursor.fetchall()
    conn.close()
    return msg
...
def update(cid,newcity):
    ...
    #用连接对象创建游标对象
    cursor = conn.cursor()
    sql = 'update tornadoAPP set city="%s" where cid="%s"'%(newcity,cid)
    cursor.execute(sql)
    conn.commit()
    conn.close()
#update('jd','shanghai')
...
def delete(cid):
    ...
    #用连接对象创建游标对象
    cursor = conn.cursor()
    sql = 'delete from tornadoAPP where cid="%s"'%(cid)
    cursor.execute(sql)
    conn.commit()
    conn.close()
#delete('tx')
